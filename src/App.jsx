...
function App() {
  ...
  const scrollSearch = () => {
    axios.get(`${apiBase}${client_id}&query=${search}&count=10`).then(
      (res) => {
  ...
  const fetchImages = () => {
    axios.get(`${apiBase}${client_id}&count=10`).then(
      (res) => {
        setImages([...images, ...res.data]);
      },
      (err) => {
  ...
  const searchImages = (e) => {
    ...
    axios.get(`${apiBase}${client_id}&query=${e.target.value}&count=10`).then(
      (res) => {
        setImages([...res.data]);
      },
      (err) => {
